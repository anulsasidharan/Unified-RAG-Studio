...
def test_load_missing_file_returns_none_with_warning(caplog: pytest.LogCaptureFixture) -> None:
    ...
    data = load_toxicity_operator_policy("/nonexistent/path/tox.json", default_extra=DEFAULT_TOXICITY_EXTRA_PATTERNS)
    assert data is None
...
def test_load_content_filter_policy(tmp_path: Path) -> None:
    ...
    data = load_content_filter_operator_policy(str(p), default_extra=DEFAULT_RETRIEVAL_FILTER_EXTRA_PATTERNS)
    assert data is not None
...
async def test_run_guarded_rag_blocks_operator_toxicity_term(
    ...
    p = tmp_path / "tox.json"
    p.write_text(json.dumps({"blocked_terms": ["stopword"], "regex_patterns": []}), encoding="utf-8")
    monkeypatch.setenv("GUARDRAILS_TOXICITY_POLICY_PATH", str(p))
    get_settings.cache_clear()
    try:
...
async def test_run_guarded_rag_warns_bias_from_policy_file(
    ...
    p = tmp_path / "bias.json"
    p.write_text(json.dumps({"regex_patterns": [r"unique_bias_test_token_12345"]}), encoding="utf-8")
