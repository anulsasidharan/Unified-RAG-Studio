"""Pipeline cost estimator driven by ``catalogs/pricing.json`` (fallback: repo ``data/pricing.json``)."""
...
from app.schemas.pipeline import CostBreakdownSchema, CostEstimateSchema, PipelineConfigurationSchema
...
def _resolver_paths(settings: Settings) -> list[Path]:
    ...
    pkg_api = Path(__file__).resolve().parents[3]  # apps/api
    return [
...
class CostEstimator:
    ...
    def estimate(self, body: CostRequest) -> CostEstimateSchema:
        ...
        emb_model = stages.embedding.model
        gen_model = stages.generation.model
        embed_1m = self._embedding_rate(emb_model)
        in_gen, out_gen = self._generation_rates(gen_model)

        embedding_per_query = (top_k * chunk_sz / 1_000_000.0) * embed_1m
        context_tokens_per_q = top_k * chunk_sz + avg_in_ass
        generation_per_query = (context_tokens_per_q / 1_000_000.0) * in_gen + (avg_out_ass / 1_000_000.0) * out_gen
        ...
        retrieval_mo = self._retrieval_read_units_monthly(vs_provider, qp_m)
        ...
        monthly = (
            totals.embedding + totals.generation + totals.reranking + totals.storage + totals.retrieval
        )
        ...
        for component, amt in (
            ("embedding_pipeline", totals.embedding),
            ("vector_storage", totals.storage),
            ("retrieval_ops", totals.retrieval),
            ("reranking", totals.reranking),
            ("generation_llm", totals.generation),
        ):
            pct = round(100.0 * amt / monthly, 2) if monthly > 0 else 0.0
            breakdown.append(
                CostBreakdownSchema(
                    component=component,
                    unit_cost=round(amt / max(qp_m, 1.0), 8) if component != "vector_storage" else round(amt, 8),
    ...
    def _generation_rates(self, model_id: str) -> tuple[float, float]:
        ...
        row = models.get(model_id)
        if isinstance(row, dict):
            return float(row.get("inputCostPer1MTokens", 0.0)), float(row.get("outputCostPer1MTokens", 0.0))
        fb = models.get("gpt-4o-mini")
        if isinstance(fb, dict):
            return float(fb.get("inputCostPer1MTokens", 0.15)), float(fb.get("outputCostPer1MTokens", 0.6))
        return 0.15, 0.6
    ...
    def _rerank_per_query_cost(self, model_id: str) -> float:
        ...
        row = models.get(model_id)
        if isinstance(row, dict):
            raw = row.get("costPer1KQueries", 0.0)
