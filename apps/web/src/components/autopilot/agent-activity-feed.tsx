  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
            Stream of <code className="rounded bg-neutral-100 px-1 py-0.5 text-xs dark:bg-neutral-800">messages</code>{' '}
            from the active build (same SSE / poll updates as Build progress). Filter and export the visible slice.
          </p>
        </div>
      ...
      {!activeBuildId ? (
        ...
        <p className="mt-6 text-sm text-muted-foreground">Build row not in memory yet — start streaming or pick another build.</p>
      ) : (
        <>
          <div className="mt-6 space-y-4">
            <div className="flex flex-wrap items-end gap-3">
              <div className="min-w-[12rem] flex-1">
                <label htmlFor={`${uid}-q`} className="flex items-center gap-1 text-xs font-medium text-neutral-700 dark:text-neutral-300">
              ...
              <div className="min-w-[10rem]">
                <label htmlFor={`${uid}-agent`} className="text-xs font-medium text-neutral-700 dark:text-neutral-300">
                  Agent
                </label>
                <select
                  id={`${uid}-agent`}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  ...
                  className="rounded-full border border-dashed border-neutral-300 px-3 py-1 text-xs text-muted-foreground hover:border-neutral-400 hover:text-neutral-800 dark:border-neutral-600 dark:hover:text-neutral-200"
                >
                  Reset filters
                </button>
              </div>
          ...
          <div className="mt-4 flex items-center justify-between gap-2 text-xs text-muted-foreground">
            <span>
              Showing <span className="tabular-nums font-medium text-neutral-800 dark:text-neutral-200">{filtered.length}</span>
              {' / '}
                ...
                {filtered.map((m, i) => (
                  ...
                  >
                    <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5 text-xs text-muted-foreground">
                      ...
                      {m.agent ? (
                        <span className="font-medium text-neutral-700 dark:text-neutral-300">{m.agent}</span>
                      ) : null}
                      <span className="uppercase tracking-wide">{m.type}</span>
                    </div>
                    <p className="mt-1 whitespace-pre-wrap break-words text-neutral-900 dark:text-neutral-100">{m.text}</p>
                  </li>
                ))}
              </ul>
            )}
