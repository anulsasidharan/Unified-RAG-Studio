...
function Collapsible({
  ...
  return (
    <div className="rounded-lg border border-neutral-200 dark:border-neutral-800">
      ...
      {open ? <div className="border-t border-neutral-200 px-4 py-3 text-sm dark:border-neutral-800">{children}</div> : null}
    </div>
  );
}
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full min-w-[480px] text-left text-xs">
        <thead>
          <tr className="border-b border-neutral-200 text-muted-foreground dark:border-neutral-700">
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.model} className="border-b border-neutral-100 dark:border-neutral-800/80">
              <td className="py-2 pr-3 font-mono text-[11px] text-neutral-800 dark:text-neutral-200">{r.model}</td>
      ...
      <div className="space-y-3">
        {d.chunking ? (
          <Collapsible title="Chunking" icon={Scissors} defaultOpen>
            <p className="text-muted-foreground">
              <span className="font-medium text-neutral-800 dark:text-neutral-200">{d.chunking.strategy}</span>
              {' · '}
            ...
            <p className="mt-2 whitespace-pre-wrap text-neutral-700 dark:text-neutral-300">{d.chunking.reasoning}</p>
        ...
        {d.embedding ? (
          <Collapsible title="Embedding" icon={Zap} defaultOpen>
            <p className="font-mono text-xs text-neutral-800 dark:text-neutral-200">{d.embedding.model}</p>
            <p className="mt-2 whitespace-pre-wrap text-neutral-700 dark:text-neutral-300">{d.embedding.reasoning}</p>
            <div className="mt-3">
        ...
        {d.retrieval ? (
          <Collapsible title="Retrieval" icon={Search} defaultOpen>
            <p className="text-neutral-800 dark:text-neutral-200">
              ...
              <span className="tabular-nums font-medium">{d.retrieval.topK}</span>
              <span className="text-muted-foreground"> · rerank </span>
              <span className="font-medium">{d.retrieval.rerankingEnabled ? 'on' : 'off'}</span>
            </p>
            <p className="mt-2 whitespace-pre-wrap text-neutral-700 dark:text-neutral-300">{d.retrieval.reasoning}</p>
            {d.retrieval.performance && Object.keys(d.retrieval.performance).length > 0 ? (
              <dl className="mt-3 grid grid-cols-2 gap-2 text-xs sm:grid-cols-3">
                {Object.entries(d.retrieval.performance).map(([k, v]) => (
                  <div key={k} className="rounded-md bg-neutral-50 px-2 py-1.5 dark:bg-neutral-900/80">
                    <dt className="text-muted-foreground">{k}</dt>
                    <dd className="tabular-nums font-medium text-neutral-900 dark:text-neutral-100">{v.toFixed(4)}</dd>
                  </div>
                ))}
              </dl>
            ) : null}
        ...
        {d.generation ? (
          <Collapsible title="Generation" icon={Sparkles}>
            <p className="font-mono text-xs text-neutral-800 dark:text-neutral-200">{d.generation.model}</p>
            <p className="mt-2 whitespace-pre-wrap text-neutral-700 dark:text-neutral-300">{d.generation.reasoning}</p>
          </Collapsible>
        ) : null}
      </div>
    );
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
            ...
            <code className="rounded bg-neutral-100 px-1 py-0.5 text-xs dark:bg-neutral-800">BuildResultSchema</code>.
          </p>
        </div>
        <button
          type="button"
