...
function qualityVsTargetsRows(quality: DashboardQualitySnapshot | undefined, targets: TargetMetrics) {
  ...
  useEffect(() => {
    ...
    setProgressSeries((prev) => {
      ...
      const next: ProgressSample[] = [
        ...prev,
        { idx: prev.length, progress: build.progress, iteration: build.iteration, ts: build.updatedAt },
      ];
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
            ...
            <code className="rounded bg-neutral-100 px-1 py-0.5 text-xs dark:bg-neutral-800">dashboardMetrics</code>{' '}
      ...
      {!activeBuildId || !build ? (
        <p className="mt-6 text-sm text-muted-foreground">
          Select an <strong>Active build</strong> in Build progress. Charts fill as snapshots arrive; evaluation and
          embedding panels appear after the worker persists <strong>stage_outputs</strong> on the build row.
        </p>
      ) : (
        <div className="mt-8 space-y-10">
          ...
          <div>
            <h3 className="flex items-center gap-2 text-sm font-semibold text-neutral-800 dark:text-neutral-200">
              <Activity className="h-4 w-4 text-primary-600 dark:text-primary-400" aria-hidden />
              Progress & iteration trend
            </h3>
            <p className="mt-1 text-xs text-muted-foreground">
              Client-side series (last {progressSeries.length} points) — one sample per unique{' '}
              <code className="rounded bg-muted px-1">progress</code> / <code className="rounded bg-muted px-1">iteration</code>{' '}
              / <code className="rounded bg-muted px-1">updatedAt</code> tick.
            </p>
            <div className="mt-4 h-56 w-full min-w-0">
              {progressSeries.length < 2 ? (
                <p className="flex h-full items-center justify-center rounded-lg border border-dashed border-neutral-200 text-sm text-muted-foreground dark:border-neutral-700">
                  Need at least two snapshots for a line — keep the build selected while the job runs.
                </p>
              ) : (
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={progressSeries} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" className="stroke-neutral-200 dark:stroke-neutral-700" />
                    <XAxis dataKey="idx" tick={{ fontSize: 11 }} label={{ value: 'Sample #', position: 'insideBottom', offset: -4, fontSize: 11 }} />
                    <YAxis yAxisId="left" domain={[0, 100]} tick={{ fontSize: 11 }} width={36} label={{ value: '%', angle: -90, position: 'insideLeft', fontSize: 11 }} />
                    <YAxis yAxisId="right" orientation="right" allowDecimals={false} tick={{ fontSize: 11 }} width={32} label={{ value: 'Iter', angle: 90, position: 'insideRight', fontSize: 11 }} />
                    <Tooltip
                      contentStyle={{ fontSize: 12 }}
                      formatter={(value: number, name: string) => [value, name === 'progress' ? 'Progress %' : 'Iteration']}
                    />
                    <Legend wrapperStyle={{ fontSize: 12 }} />
                    <Line yAxisId="left" type="monotone" dataKey="progress" name="progress" stroke="hsl(var(--primary))" strokeWidth={2} dot={false} isAnimationActive={false} />
                    <Line yAxisId="right" type="stepAfter" dataKey="iteration" name="iteration" stroke="#64748b" strokeWidth={1.5} dot={false} isAnimationActive={false} />
                  </LineChart>
                </ResponsiveContainer>
              )}
            </div>
          ...
          <div>
            <h3 className="flex items-center gap-2 text-sm font-semibold text-neutral-800 dark:text-neutral-200">
              <Wallet className="h-4 w-4 text-primary-600 dark:text-primary-400" aria-hidden />
              Cost & latency SLOs
            </h3>
            <div className="mt-4 grid gap-3 sm:grid-cols-3">
              <div className="rounded-lg border border-neutral-200 bg-neutral-50/50 p-4 dark:border-neutral-800 dark:bg-neutral-900/40">
                <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Budget cap</p>
              ...
              <div className="rounded-lg border border-neutral-200 bg-neutral-50/50 p-4 dark:border-neutral-800 dark:bg-neutral-900/40">
                <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Latency requirement</p>
                <p className="mt-1 text-lg font-semibold tabular-nums text-neutral-900 dark:text-neutral-50">
                  {latencySlo !== undefined ? `${latencySlo} ms` : 'Not set'}
                </p>
              </div>
              <div className="rounded-lg border border-neutral-200 bg-neutral-50/50 p-4 dark:border-neutral-800 dark:bg-neutral-900/40">
                <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Eval proxy latency</p>
                <p className="mt-1 text-lg font-semibold tabular-nums text-neutral-900 dark:text-neutral-50">
          ...
          <div>
            ...
            <div className="mt-4 h-64 w-full min-w-0">
              {qualityRows.length === 0 ? (
                <p className="flex h-full items-center justify-center rounded-lg border border-dashed border-neutral-200 text-sm text-muted-foreground dark:border-neutral-700">
                  No evaluation metrics in <code className="mx-1 rounded bg-muted px-1">dashboardMetrics.quality</code> yet.
                </p>
              ) : (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={qualityRows} layout="vertical" margin={{ top: 8, right: 24, left: 8, bottom: 8 }}>
                    <CartesianGrid strokeDasharray="3 3" className="stroke-neutral-200 dark:stroke-neutral-700" horizontal={false} />
                    <XAxis type="number" domain={[0, 1]} tick={{ fontSize: 11 }} />
                    <YAxis type="category" dataKey="metric" width={120} tick={{ fontSize: 11 }} />
                    <Tooltip formatter={(v: number) => [v.toFixed(3), '']} />
                    <Legend wrapperStyle={{ fontSize: 12 }} />
                    <Bar dataKey="value" name="Observed" fill="hsl(var(--primary))" radius={[0, 4, 4, 0]} maxBarSize={22} />
                    <Bar dataKey="target" name="Target" fill="#94a3b8" radius={[0, 4, 4, 0]} maxBarSize={22} />
                  </BarChart>
                </ResponsiveContainer>
              )}
            </div>
          ...
          <div>
            <h3 className="flex items-center gap-2 text-sm font-semibold text-neutral-800 dark:text-neutral-200">
              <Timer className="h-4 w-4 text-primary-600 dark:text-primary-400" aria-hidden />
              Embedding benchmark latency
              {dm?.selectedEmbeddingLabel ? (
                <span className="ml-2 text-xs font-normal text-muted-foreground">Selected: {dm.selectedEmbeddingLabel}</span>
              ) : null}
            </h3>
            <div className="mt-4 h-64 w-full min-w-0">
              {latencyRows.length === 0 ? (
                <p className="flex h-full items-center justify-center rounded-lg border border-dashed border-neutral-200 text-sm text-muted-foreground dark:border-neutral-700">
                  No embedding candidates in <code className="mx-1 rounded bg-muted px-1">dashboardMetrics.embeddingBenchmarks</code>.
                </p>
              ) : (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={latencyRows} margin={{ top: 8, right: 8, left: 8, bottom: 48 }}>
                    <CartesianGrid strokeDasharray="3 3" className="stroke-neutral-200 dark:stroke-neutral-700" />
                    <XAxis dataKey="model" tick={{ fontSize: 10 }} interval={0} angle={-28} textAnchor="end" height={56} />
                    <YAxis tick={{ fontSize: 11 }} width={48} label={{ value: 'ms / text', angle: -90, position: 'insideLeft', fontSize: 11 }} />
                    {latencySlo !== undefined ? (
                      <ReferenceLine y={latencySlo} stroke="#f59e0b" strokeDasharray="4 4" label={{ value: 'SLO', fill: '#f59e0b', fontSize: 11 }} />
                    ) : null}
                    <Tooltip
                    ...
                    />
                    <Legend wrapperStyle={{ fontSize: 12 }} />
                    <Bar dataKey="latencyMs" name="Latency (ms)" fill="#0ea5e9" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              )}
            </div>
          ...
          <div>
            <h3 className="flex items-center gap-2 text-sm font-semibold text-neutral-800 dark:text-neutral-200">
              <BarChart3 className="h-4 w-4 text-primary-600 dark:text-primary-400" aria-hidden />
              Retrieval summary
            </h3>
            {dm?.retrieval?.strategy ? (
              ...
              <p className="mt-2 text-sm text-muted-foreground">No retrieval summary on the latest snapshot.</p>
            )}
            <div className="mt-4 h-52 w-full min-w-0">
              {perfEntries.length === 0 ? (
                <p className="flex h-full items-center justify-center rounded-lg border border-dashed border-neutral-200 text-sm text-muted-foreground dark:border-neutral-700">
                  No <code className="mx-1 rounded bg-muted px-1">retrieval.performance</code> map yet.
                </p>
              ) : (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={perfEntries} margin={{ top: 8, right: 8, left: 8, bottom: 32 }}>
                    ...
                    <Bar dataKey="value" name="Retrieval metric" fill="#8b5cf6" radius={[4, 4, 0, 0]} maxBarSize={48} />
                  </BarChart>
                </ResponsiveContainer>
              )}
            </div>
