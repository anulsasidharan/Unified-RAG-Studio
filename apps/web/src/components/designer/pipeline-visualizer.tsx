...
function MermaidDiagram({
  ...
  return (
    <div className={cn('relative min-h-[120px]', className)}>
      {busy ? (
        <div className="bg-muted/40 absolute inset-0 z-10 flex items-center justify-center rounded-md">
...
function ZoomToolbar({
  ...
  return (
    <div
      className={cn(
        'bg-background/95 flex flex-wrap items-center gap-1 rounded-md border border-neutral-200 px-1 py-0.5 text-xs dark:border-neutral-700',
        className,
      )}
      role="toolbar"
    ...
    >
      <button
        ...
        className="text-muted-foreground hover:bg-muted hover:text-foreground inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-md transition-colors disabled:pointer-events-none disabled:opacity-40"
        title="Zoom out"
        aria-label="Zoom out"
      >
      ...
      <button
        type="button"
        onClick={onReset}
        className="text-muted-foreground hover:bg-muted hover:text-foreground min-w-[3.25rem] rounded-md px-2 py-1 font-mono text-[11px] tabular-nums"
...
export function PipelineVisualizer({
  ...
  const summaryPanel = (
    <div className="space-y-3">
      <div>
        ...
        <p className="text-muted-foreground mt-1 line-clamp-2 text-xs leading-snug" title={oneLine}>
          {oneLine}
        </p>
      </div>

      <ul className="border-primary-600/30 text-muted-foreground dark:border-primary-400/30 space-y-1 border-l-2 pl-3 text-[11px] leading-relaxed">
        {bullets.map((line) => (
          <li key={line}>{line}</li>
        ))}
      </ul>
      ...
      ? createPortal(
          <div className="fixed inset-0 z-[200]">
            ...
            <div
              ...
              className={cn(
                'bg-background absolute z-10 flex flex-col overflow-hidden rounded-xl border border-neutral-200 shadow-2xl dark:border-neutral-700',
                pipMaximized
                  ? 'left-4 top-4 h-[calc(100vh-2rem)] max-h-[calc(100vh-2rem)] w-[calc(100vw-2rem)] max-w-[calc(100vw-2rem)]'
                  : 'bottom-6 right-6 h-[min(72vh,560px)] min-h-[220px] w-[min(92vw,640px)] min-w-[min(100%,320px)] resize',
              )}
            >
              <div className="bg-muted/30 flex shrink-0 items-center justify-between gap-2 border-b border-neutral-200 px-3 py-2 dark:border-neutral-700">
  ...
  return (
    <section
      ...
      id={id}
      className={cn(
        'bg-card/40 w-full shrink-0 scroll-mt-4 border-t border-neutral-200 py-4 dark:border-neutral-800',
        className,
      )}
      ...
      <div className="mx-auto w-full max-w-[1920px] px-4 sm:px-6 lg:px-8">
        <div className="space-y-1">
          <div className="flex flex-wrap items-start justify-between gap-2">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                type="button"
                onClick={openPip}
                className="bg-background text-muted-foreground hover:bg-muted hover:text-foreground inline-flex items-center gap-1.5 rounded-md border border-neutral-200 px-2.5 py-1.5 text-xs font-medium shadow-sm transition-colors dark:border-neutral-600"
          ...
          <p className="text-muted-foreground text-[11px] leading-snug">
            The drawing starts empty on Cloud Provider; each stage you move into adds the next
            blocks. Labels use your draft values; optional stages (rerank, routing, memory,
            evaluation, guardrails, human-in-the-loop) appear once you reach those steps.
          </p>
        </div>

        {/* Full-width strip: summary + diagram side-by-side on large screens */}
        <div className="bg-muted/20 mt-4 min-h-0 rounded-lg border border-neutral-200 lg:grid lg:max-h-[min(560px,52vh)] lg:grid-cols-[minmax(200px,22rem)_minmax(0,1fr)] lg:divide-x lg:divide-neutral-200 lg:overflow-hidden dark:border-neutral-700 dark:lg:divide-neutral-700">
          <div className="max-h-[min(420px,45vh)] min-h-0 overflow-y-auto p-3 lg:max-h-none">
            {summaryPanel}
          </div>
          <div className="min-h-0 min-w-0 border-t border-neutral-200 p-3 lg:overflow-auto lg:border-t-0 dark:border-neutral-700">
            <MermaidDiagram
              key={definition}
              definition={definition}
              zoom={inlineZoom}
