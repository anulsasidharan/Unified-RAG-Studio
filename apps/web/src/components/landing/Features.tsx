import { LayoutTemplate, Bot, Cloud, FlaskConical, DollarSign, Rocket } from 'lucide-react';
...
const colorMap: Record<string, { bg: string; icon: string; border: string; glow: string }> = {
  primary: {
    ...
    glow: 'hover:border-primary-200 hover:shadow-primary-100/60',
  },
  purple: {
    bg: 'bg-purple-50',
    icon: 'text-purple-600',
    border: 'border-purple-100',
    glow: 'hover:border-purple-200 hover:shadow-purple-100/60',
  },
  sky: {
    bg: 'bg-sky-50',
    icon: 'text-sky-600',
    border: 'border-sky-100',
    glow: 'hover:border-sky-200 hover:shadow-sky-100/60',
  },
  success: {
    bg: 'bg-success-50',
    icon: 'text-success-600',
    border: 'border-success-100',
    glow: 'hover:border-success-200 hover:shadow-success-100/60',
  },
  warning: {
    bg: 'bg-warning-50',
    icon: 'text-warning-600',
    border: 'border-warning-100',
    glow: 'hover:border-yellow-200 hover:shadow-yellow-100/60',
  },
  danger: {
    bg: 'bg-danger-50',
    icon: 'text-danger-600',
    border: 'border-danger-100',
    glow: 'hover:border-red-200 hover:shadow-red-100/60',
  },
};
    ...
    <section id="features" className="bg-neutral-50/60 py-24">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          >
            <div
              className={`mb-5 inline-flex h-14 w-14 items-center justify-center rounded-2xl ${heroColors.bg}`}
            >
              <hero.icon className={`h-7 w-7 ${heroColors.icon}`} />
            </div>
            <h3 className="font-display mb-3 text-xl font-bold text-neutral-900">{hero.title}</h3>
            ...
            <div className="pointer-events-none absolute bottom-6 right-6 flex items-center gap-1.5 opacity-20">
              {['Retrieve', 'Augment', 'Generate'].map((s, i) => (
                <div key={s} className="flex items-center gap-1.5">
                  <span className="bg-primary-600 rounded-md px-2 py-1 text-[9px] font-bold text-white">
                    {s}
                  </span>
          ...
          {rest.map(({ icon: Icon, title, description, color }) => {
            ...
            return (
              ...
              >
                <div
                  className={`mb-4 inline-flex h-11 w-11 items-center justify-center rounded-xl ${c.bg}`}
                >
                  <Icon className={`h-5 w-5 ${c.icon}`} />
                </div>
                <h3 className="font-display mb-2 font-semibold text-neutral-900">{title}</h3>
